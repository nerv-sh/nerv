...
| 6 | `docs/spec-conversion-policy.md` (v1.4) | TS spec → JSON Tier A/B/C 정책 + Fig `loadSpec.ts` 포팅 + rquickjs Tier C (2026-07-08 출시 동봉, `release.yml --features quickjs`). v1.4 = `limited_args`/§5.1 힌트 UX 폐기 반영 |
...
- ✅ **SpecRegistry LRU bound** (`SPEC_CACHE_CAP=16`): lazy 캐시가 무제한 성장하던 것 (aws 파싱본 ~290MB RSS / cold load ~220ms — 실측) → positive entry 16개 cap. lookup/insert 가 최소-tick positive evict (per-entry `AtomicU64` tick, read-lock 하에서 hit 마다 bump → active spec 은 절대 evict 안 됨). negative entry 는 byte-free 라 미집계. eager `load_dir`(doctor 전용)은 면제. 라이브 검증: aws + 16 distinct spec churn 후 aws cold reload / interleaved 사용 시 warm 유지. 무한 성장 차단, peak = 동시 working set. 후속 레버(3.6× 파싱 blowup 축소 — `Box<str>`/interned desc)는 defer.
...
- ✅ **release.yml 워크플로**: `v*.*.*` tag push / `workflow_dispatch` / **main 머지** → macos-14 빌드 + tarball (sha256) + GitHub Release 생성. `gh release create … --clobber` 로 idempotent. **auto-release (2026-07-16)**: `push: branches: [main]` 트리거 + 저렴한 ubuntu `gate` job 이 tag/should_release 판정 — main 머지 시 tag = `v{Cargo.toml workspace version}`, 해당 릴리스가 아직 없고 머지 커밋에 `[skip release]` 없을 때만 릴리스 (⇒ 출시 = Cargo.toml 버전 bump, 그 외 머지는 no-op → macOS 빌드 낭비 없음). gate→build→release 가 **한 워크플로 run** 안에서 실행되므로 생성된 tag 가 무언가를 재트리거할 필요 없음.
- ✅ **완성 응답성 배치 (2026-07-16)** — 대형/느린 제너레이터로 인한 keystroke stutter 3종 근절, 라이브 벤치 검증:
  - **timeout 제너레이터 negative-cache** (`nerv-engine::complete::cached_generator_lines`): `brew upgrade` 의 `brew outdated -q` (~1.3s) 가 800ms `GENERATOR_TIMEOUT_MS` 를 매번 초과 → `None` 반환 → 캐시 안 됨 → 키입력마다 재spawn + ~800ms 블록. 캐시 값 타입 `Vec<String>` → `Option<Vec<String>>` 로 바꿔 miss(`None`)도 memoize. keystroke당 → 5s TTL당 1회.
  - **전송 suggestion 캡 500** (`nerv-daemon::engine_complete`): `brew install` 이 16,914 formulae 전량을 위젯에 전송 → zsh 위젯이 O(N) 스캔으로 팝업 박스 크기 계산 (`__nerv_measure_items`, ~270ms/keystroke = stutter). frecency/dotnav ranking **후** `MAX_SUGGESTIONS=500` truncate. 위젯 split+measure ~6ms. 모든 명령 공통 (단일 choke). prefix-filter 가 `complete_in` 안에서 선행되므로 cap 은 broad browse tail 만 자름.
  - **async 제너레이터** (`cached_generator_lines` hybrid): cold 제너레이터가 keystroke 를 블록하던 것 (brew install 739 / docker run 282 / brew outdated 814ms) → 백그라운드 populator 스레드 + `GENERATOR_SYNC_WAIT=50ms` sync window. fast gen (git branch/checkout, ssh, pwd — <50ms) 은 같은 keystroke 반환, slow 는 defer → 다음 keystroke 에 landing. stale-while-revalidate + `GENERATOR_INFLIGHT` dedup (Drop-guard 로 panic 시도 해제) + store→release→send 순서로 fast-path 결정적. compute `Send + 'static` (owned script/cwd), 명시적 prewarm scope 삭제 (populator 가 이미 병렬).
  - 결과: warm <13ms, rapid typing <8ms/key, cold first-hit ~50ms (이전 282–814ms). 728 test green + 3 회귀 test (negative-cache / cap / async concurrent). ~~미해결: 대형 spec 최초 parse (aws `~245ms`)~~ → **아래 async 스펙 로드로 해결 (2026-07-17)**.
  - **async 스펙 로드 (2026-07-17)**: 마지막 keystroke-freeze 근원. 첫 `aws ` 키가 셸을 **~260ms 얼림** — aws spec = 7.4MB gz(2위 gcloud 1.4MB, 나머지 <132KB, 압도적 outlier), `SpecRegistry::lookup` 이 파싱을 **동기**로 돌려 CLI 브리지 500ms 예산 안에서 위젯이 블록. generator 하이브리드를 lookup 에 이식: **공유 원시 `spawn_populator_and_maybe_wait` 추출** (generator memo + spec loader 양쪽이 재사용 — spawn+Drop-guard+recv_timeout, 복사 divergence 제거) → cold/stale lookup 은 백그라운드 파싱 스레드 + `SPEC_LOAD_SYNC_WAIT=50ms` window. **빠른 ~700 스펙은 같은 키에 착지(무 flash), aws/gcloud(>50ms)만 defer → 첫 키 empty, 다음 키 full**. `inflight: Arc<Mutex<HashSet>>` dedup 으로 빠른 콜드 타이핑(`aws s3api …`)이 45MB 파싱 동시 N개 spawn 하던 것 차단. mtime hot-reload 는 이제 stale-while-revalidate(파일 변경 시 구 spec 1키, 다음 키 신규 — `lookup_hot_reloads` poll 로 갱신). struct 필드 `cache`/`next_tick` → Arc(Deref 투명), `load_from_disk`/`evict_overflow` → 연관함수(no-self, 스레드 재사용). 실측 cold 첫 키: git 0.01s/40줄(회귀 없음) · aws 0.05s/0줄(defer)→다음 0.00s/409줄. 730 test green + 회귀 `populator_serves_fast_and_stale_but_defers_slow`(fast 착지/slow defer/stale 즉시 3케이스 타이밍 판별) + 기존 `lazy_lookup_reads_from_disk_on_miss`(cold git 첫 키 non-empty = 713 회귀 가드). 남은 defer: aws 파싱 5초 TTL 재spawn 낭비는 spec 아닌 generator 얘기(별개), binary 사전파싱 포맷(메모리도↓)은 큰 레버 defer.
  - **revalidation 무대기 (2026-07-17)**: async 배치 이후에도 warm 명령이 **5초 TTL 경계마다 한 키에서 ~120ms hitch** (brew install 실측). 원인 = `cached_generator_lines` 가 positive stale 를 손에 쥐고도 `recv_timeout(50ms)` sync window 를 소비 — 느린 gen (`brew formulae` 630ms) 은 매번 timeout → 생성기당 50ms (brew 2개=~100ms, aws 체인은 150-200ms). 수정 = `stale.is_some()` 이면 sync window 건너뛰고 stale 즉시 반환 + 백그라운드 refresh. window 는 cold/miss (stale None) 에서만 유지 (빈 화면 flash 방지). **모든 느린 생성기 공유 경로** — brew/docker/aws 전부 warm 후 TTL 넘어도 영구 무블록. 실측 brew revalidation 0.12s→0.01s (여전히 500줄 non-empty). 회귀 test `revalidation_serves_stale_without_waiting_sync_window` (만료 positive-stale seed + 150ms compute → stale 반환 <25ms; "stale 반환"만으론 판별 불가 → 타이밍이 판별). 5초 TTL 자체(정적 16k `brew formulae` 를 5초마다 백그라운드 재spawn — off-path 낭비) 축소는 별도 efficiency 레버, defer. 729 test green.
...
- ✅ **Homebrew tap 인프라**: `packaging/homebrew/nerv.rb` Formula 템플릿 (ARM-only `aarch64-apple-darwin`, `brew services` 통합). `.github/workflows/homebrew-bump.yml` 가 `nerv-sh/homebrew-tap` 의 Formula 를 버전+sha256 갱신. **트리거 (2026-07-16 정정)**: `GITHUB_TOKEN` 으로 만든 릴리스는 `release:published` 이벤트를 못 쏜다 (GitHub 이 토큰-유발 워크플로 cascade 를 recursion 방지로 차단) → release.yml 이 `gh workflow run homebrew-bump.yml -f tag=…` 로 **명시적 dispatch** (`workflow_dispatch` 는 이 규칙 예외). `release:published` auto-trigger 는 fallback 로 유지하되 실질 경로는 명시적 dispatch. 사용자 액션: tap repo 생성 + `HOMEBREW_TAP_TOKEN` PAT secret 추가.
...
- ✅ **Tier C executor 배선** (`feature = "quickjs"` opt-in): `nerv-quickjs` 스캐폴드 (rquickjs ~1MB sandbox, `eval_isolated` + `eval_with_budget` 200ms 기본) → `nerv-engine::tier_c::execute_custom_source` 헬퍼 → `complete.rs` 의 `Generator::Custom { source: Some(_), .. }` arm wire-up. ts-to-json 의 `captureClosureSource` 가 closure `toString()` 을 IIFE 형태로 감싸 (`(<fn>)(globalThis.__nerv_tokens, () => Promise.resolve(""))`) 32KB cap 적용해 emit. 715 spec 변환 → closure source 캡처 (corpus 1125, aws 749 지배적). ⚠️ **capture ≠ execution**: 2026-06-06 첫 e2e 는 실행률 **0%** (async 未drain + `__awaiter` 미정의 + shell stub) → **2026-07-07 머신러리 3종 수정** (eval_resolved job-queue drain / tslib helper 주입 / 실 executeShellCommand host binding) → **실행률 82% settle / 62% ok_ge1** (aws 746/749). **2026-07-08: 출시 빌드에 동봉** (`release.yml --features nerv-cli/quickjs,nerv-daemon/quickjs`, +0.78MB) — JS 머신러리 실측 2-3ms < 25ms 예산. 상세 = `docs/findings/tier-c-quickjs-e2e.md`. Custom arm 은 well-known 회복 (aws_list 89 / kubectl_resources 86 / package_json_scripts 28 / ssh_hosts 9 / 외) 통과 후 마지막 fallback 으로만 동작. Soft-fail: tier_c None → next generator → smart filepaths fallback. 5 dispatch test + 1 default-build 호환 test.
...
- aws 624 closure-form generators 중 89 = `aws_list` 회복, 나머지 = `Generator::Custom { source }` 로 캡처됨 → quickjs 빌드에서 실행. ~~출시 결정 확정 (2026-06-07): Tier C 실행률 e2e = 0% → production 기본 OFF, 출시 바이너리 미동봉~~ → **뒤집힘 (2026-07-08)**: 2026-07-07 머신러리 수정으로 실행률 0% → **82% settle / 62% ok_ge1** (aws 746/749). 재측정으로 두 우려 해소 — (1) 성능: JS 머신러리 stage 별 실측 = new 175µs + helpers 295µs + shim 185µs + eval 1.5-2.5ms = **총 2-3ms** (25ms 예산 내, 결과캐시/런타임재사용 불필요), (2) 기능: 출시 바이너리엔 quickjs 가 아예 없어(release.yml 플래그 누락) aws 회복이 유저에게 도달 못 하던 것 확인 → **`release.yml` 에 `--features nerv-cli/quickjs,nerv-daemon/quickjs` 동봉** (+0.78MB). 남은 tail(cargo 0/44 / chezmoi/nx/asdf 등 니치툴) = module-level 헬퍼 미포착 (`'separator'/'getSuggestions' is not defined`) → esbuild per-spec 번들러 필요 (큰 레버·수익체감, deferred). 상세 = `docs/findings/tier-c-quickjs-e2e.md` §2026-07-08.
...
| 위젯 Enter/센티넬 | 팝업 첫 행 = `↩ Immediately execute` sentinel. `__NERV_SELECTED=0`=sentinel(Enter=라인 실행), `1..N`=항목(Enter=삽입). 기본 선택: 빈 토큰(trailing space)→0, 부분 입력→1. **예외 (디렉터리 = Enter runs, Tab drills)**: 선택 항목의 insertion 이 `/`로 끝나면 (디렉터리 — `../`/`./`/`cli/`/`src/`) Enter=삽입+실행 (한 keypress). 디렉터리 선택 = "거기로 간다" 의미 → 두 번 안 눌러도 됨. 더 깊이 내려가려면 Tab (삽입 후 다음 레벨 팝업). 비-디렉터리(서브커맨드/플래그/브랜치)는 insert-only — bare `git checkout` 조기 실행 방지. 판정 = `[[ $insertion == */ ]]`. **인라인 ghost 는 히스토리 우선** (`__nerv_history_ghost`), 없으면 spec 토큰. `complete()` 가 `insertion==prefix` no-op 제안 drop. 이 계약 바꾸면 dogfood UX 회귀 — e2e-zle-{enter,history,dotnav} 로 검증 | `shell-integrations/zsh/_nerv.zsh` / `nerv-engine/src/complete.rs` |
...
| JS 엔진 | **deno_core 임베드 금지** — `rquickjs` (~1 MB) 만. Tier C 는 **출시 빌드에 동봉** (`release.yml` 이 `--features nerv-cli/quickjs,nerv-daemon/quickjs` 로 빌드, 2026-07-08 결정. 근거: JS 머신러리 2-3ms < 25ms 예산 + aws 92% 회복. `docs/findings/tier-c-quickjs-e2e.md`). **well-known 패턴은 Rust-native 회복 우선** (예: `Generator::PackageJsonScripts` — npm scripts 추출은 closure 우회), Tier C 는 마지막 fallback | PLAN §0.2 / §7 |
