...
| 위젯 Enter/센티넬 | 팝업 첫 행 = `↩ Immediately execute` sentinel. `__NERV_SELECTED=0`=sentinel(Enter=라인 실행), `1..N`=항목(Enter=삽입). 기본 선택: 빈 토큰(trailing space)→0, 부분 입력→1. **예외 (dotnav)**: 선택 항목의 insertion 이 `./`/`../` 핀이면 Enter=삽입+실행 (한 keypress) — dotnav 는 drill 대상 아닌 terminal nav target (`cd ..`+Enter 이 두 번 눌러야 하던 것 수정). 그 외 항목은 insert-only (Tab/2차 Enter 로 drill). **인라인 ghost 는 히스토리 우선** (`__nerv_history_ghost`), 없으면 spec 토큰. `complete()` 가 `insertion==prefix` no-op 제안 drop. 이 계약 바꾸면 dogfood UX 회귀 — e2e-zle-{enter,history,dotnav} 로 검증 | `shell-integrations/zsh/_nerv.zsh` / `nerv-engine/src/complete.rs` |
