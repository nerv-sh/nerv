...
- **스큐가 남긴 shell 이름 행은 프루닝된다 — v1.8.** 버전 스큐(§3.6.7) 기간의 데몬은
  ...
  이름 집합이고, 실행 시점은 **첫 PATH 스캔 완료**와 **shell 이름 등록** 두 곳이다
  (먼저 도는 쪽이 지우고 나머지는 no-op). 소스가 빈 상태(등록 전)로 프루닝이 돌면
  **아무 행도 지우지 않는다** — 등록은 메모리 전용이라 재시작마다 사라지므로, 등록
  이후의 패스가 유일한 기회다(이 순서 역전이 회귀 시나리오). PATH 에 실재하는 spec
...
*v1.8 — §3.6.3 shell 이름 행 프루닝 조항 신설 (2026-09-21). 등록된 shell 함수·alias 이름을 아는 데몬이 스큐 기간에 쌓인 해당 miss 행을 첫 PATH 스캔 완료·이름 등록 시점에 원자 재작성으로 정리한다. 빈 소스(등록 전) 프루닝은 no-op, PATH 실재 바이너리 행은 보존. 5종 카탈로그 / 메시지 톤 / exit code 규약 무변경.*
