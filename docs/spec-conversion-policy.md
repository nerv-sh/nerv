...
| 옵션 행 다중 항목 | 한 행이 `X or Y` 산문 대안(`-irsD or -c command or -O shopt_option`, bash)이면 ` or ` 로 나눠 각각 옵션. 단일대시 + 알파벳만 + 대소문자 혼합 3자 이상(`-irsD`)은 짧은 플래그 묶음 → 글자마다 옵션 (go 식 `-verbose` 는 한 케이스라 제외). 탭 정렬 help 는 탭을 컬럼 브레이크로 (2026-09-16, fixture `help/bash.txt`) |
