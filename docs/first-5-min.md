...
> **v0.6 정합**: 본 시나리오의 12+0.5 단계는 v0.5 그대로. PRD v0.6 의 엔진 전환 (자작 → Fig Rust crates 흡수 + TS Rust 포팅) 은 *내부 구현 변경*, 사용자 체감 동일. 단 동적 힌트 (4/7/11/12 단계) 의 v1.1 회복 경로가 **rquickjs opt-in** 으로 명시됨 (§8 갱신).
...
v1.1 동적 generator 도입 (PRD v0.6 §5.7 = **rquickjs opt-in**)
후 본 12단계의 4/7/11/12를 *실제 동적 추천* 으로 교체. 그 시점의
시나리오는 별도 `docs/first-5-min-v1.1.md` 로 작성.
...
- rquickjs 는 **opt-in** (config 의 `[runtime] dynamic_specs = true`) — 기본은 정적 힌트 유지. 사용자가 동적 완성을 명시 활성화해야 4/7/11/12 단계의 응답이 바뀜.
